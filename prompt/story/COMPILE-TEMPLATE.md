...
2. **그 위에서 엔딩 3개를 서로 다른 사건 경유·해소 조합에 뿌리내리게 만든다.** 각 엔딩의 `ending_requirement`는 어떤 사건을 어떻게 풀어야 그 엔딩인지 **사건 이름을 집어** 쓰되, 순서와 무관하게 쓴다.

엔딩 3개 조건이 지켜야 할 두 성질:

- **상호배타**: 세 조건이 서로 겹치지 않는다.
- **총망라**: 세 조건을 합치면 **결말이 날 만한 모든 상태를 빈틈없이 덮는다.** 조건을 "3명 살리면 해피 / 2명 노말 / 다 죽으면 배드"처럼 딱 떨어지는 개수·조합으로 세지 않는다 — 그러면 사이 경우(1명 남음)가 빠져 결말인데도 엔딩이 없다. 대신 **성취 정도의 스펙트럼**으로 나눈다: 온전한 성공(HAPPY) / 그에 못 미치는 **그 사이 전부**(NORMAL) / 파멸·실패(BAD). NORMAL이 중간대를 통째로 받아 사각지대가 없게 한다.
...
- `endings`: 스토리의 엔딩 **정확히 3개**로, `ending_type`이 `HAPPY`·`NORMAL`·`BAD`인 엔딩을 **각각 하나씩** 만든다(위 결속 설계를 따른다). 각 엔딩은 다음을 채운다.
  - `ending_type`: `HAPPY` / `NORMAL` / `BAD` 중 하나(셋이 각 1개, 중복 금지).
  - `ending_requirement`: 이 엔딩에 도달하는 조건. **최소 채팅 턴 수** + **스토리 핵심 목적 달성 여부** + **거쳐야 할 주요 사건과 그 해소**를 담는다. 이른 종결을 막도록 세 엔딩 모두(BAD 포함) 최소 턴 조건을 둔다.
  - `ending_epilogue`: 조건을 충족했을 때 엔딩 응답(에필로그)을 **어떤 방향으로 생성할지** 안내한다. 완성된 결말 본문을 미리 쓰지 않는다 — 도달 시점에 사용자의 실제 행적을 반영해 생성될 것이기 때문이다. 반드시 **"사용자가 실제로 한 행동들을 되짚어, 그 행동이 세계를 어떻게 바꿨는지 보여주라"는 지시**를 담는다. BAD도 실패 화면이 아니라 여운 있는 비극으로 안내한다.
...
- 엔딩이 `HAPPY`·`NORMAL`·`BAD` 각 1개(총 3개)인가?
- 세 엔딩 조건이 서로 겹치지 않고(상호배타), 합치면 결말이 날 만한 모든 상태를 빈틈없이 덮는가(총망라 — NORMAL이 중간대를 흡수)?
- 각 엔딩 조건이 주요 사건 이름을 집어 사건 경유·해소로 뿌리내렸는가?
- 각 `ending_epilogue`가 "사용자 행적을 되짚어 세계 변화를 보여주라"는 지시를 담았는가?
