...
async def test_502_after_two_refills(monkeypatch: pytest.MonkeyPatch) -> None:
    """두 번 다시 받아도 인물이 계속 빠지면 502로 막고, 재호출 횟수를 예외에 싣는다."""
    calls: list[str] = []
    ...
    async def fake_complete(system: str, user: str, *args: object, **kwargs: object):
        ...
        if label.startswith("storylines-refill"):
            data = {
                "stories": [
                    {"id": 2, "storyline": "여전히 없는 2편", "recommended_infos": ["가", "나", "다"]}
                ]
            }
        else:
    ...
    with pytest.raises(HTTPException) as ei:
        await story_llm.generate_storylines(system, user, required_names=["서린"])

    assert ei.value.status_code == 502
    assert calls == ["storylines", "storylines-refill#1", "storylines-refill#2"]
    assert getattr(ei.value, "retry_count", None) == 2


async def test_refill_breaking_contract_is_blocked(monkeypatch: pytest.MonkeyPatch) -> None:
    """재호출이 못 쓸 편을 데려오면(추천 정보 부족) 502로 막는다 — 조립에서 500으로 터지지 않게."""
