...
- **설명은 이 코드를 처음 보는 팀원이 읽는다고 생각하고 씁니다.** 이 팀은 에이전트 설명만 보고
  결정하기 때문에, 다시 물어보는 한 번이 그대로 시간 낭비입니다. 설명할 때는 이 순서로 씁니다.
  1. 첫 문장이 답입니다. 뭘 했는지, 또는 뭐가 문제인지.
  2. 실제로 무슨 일이 일어나는지 짧게 씁니다. 뭐가 들어가고, 뭐가 일어나고, 그래서 뭐가 곤란해지는지.
     비유 대신 실제 동작을 씁니다.
  3. 영향받는 것을 전부 씁니다. 파일, 티켓, 다른 레포, 뒤에 할 일. "등"으로 줄이지 않습니다.
  - 평소 대화에서 안 쓰는 말(드리프트, 앵커, 정본 같은 팀 내부 이름)은 처음 나올 때 풀어 쓰거나
    흔한 말로 바꿉니다. 기준: 같은 팀 비개발자가 그 문장을 그대로 읽고 이해하는가.
  - 선택지를 늘어놓고 되묻지 말고, 추천 하나를 이유와 함께 냅니다.
  - 리뷰·검토 결과의 지적 항목은 하나마다 두 줄로 씁니다. 첫 줄은 무엇이 문제인지 쉬운 말로,
    둘째 줄은 `이유:`로 시작해 왜 그게 문제인지(그대로 두면 무슨 일이 생기는지) 쉬운 말로.
