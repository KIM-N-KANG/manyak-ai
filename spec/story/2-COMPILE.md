...
컴파일 프롬프트는 카드를 작성하기 전에 선택 스토리라인의 인물을 입력 목록에 연결하도록 지시합니다. 이름이 정해진 입력을 먼저 연결하고, 이름 미정 입력은 아직 연결되지 않은 이야기 속 인물로 채웁니다. 호칭만 있는 인물도 포함하며 관계·역할·행적을 유지합니다. 사용자 지정 값은 우선 적용하고, 카드와 본문에서는 같은 인물을 일관되게 표현합니다. 카드 블록 보완에서도 잘못 생성된 새 인물 대신 원래 스토리라인을 기준으로 삼습니다. 별도 인물 명단을 API로 주고받거나 이 연결의 의미를 코드로 판정하지는 않습니다.
...
- **이미지 모델**: 기본값은 `gpt-image-2.5-flare`이며 `IMAGE_MODEL` 환경변수로 변경할 수 있습니다. 인물·표지·채팅 자식 이미지가 같은 설정을 사용합니다. 이미지 모델은 텍스트 LLM과 같은 3층 구조(호출부 → 모델 특성 → SDK 어댑터)를 따릅니다.
