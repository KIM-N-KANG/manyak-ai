...
- 기능·버그·설계 변경을 담은 **커밋마다**(PR 이후 후속 커밋 포함) 코드만 바꾸고 정본을 방치하지 말고, 다음 세 문서를 함께 갱신합니다. (이 갱신은 훅 유무와 무관한 의무입니다 — 보조로 `.claude/settings.json`의 `PostToolUse` 훅이 커밋 직후 일깨우지만, 훅이 없어도 수동으로 갱신해야 합니다.)
  - `ai-spec` — 설계가 바뀌었으면 먼저 갱신합니다(설계 결정은 여기에만 적음). fe/be 접점(§7)을 건드리면 어떤 계약에 영향 주는지 명시하고 동기화 필요를 알립니다.
  - `ai-plan` — 바뀐 설계가 어디에 구현됐는지/될지 현 구현 맵·계획을 맞춥니다.
  - `ai-ops` — §2 패치노트에 티켓(KNK)·증상·원인·해결을, §4 PR 이력에 PR 한 줄을 남깁니다.
- 진입점(설계 먼저냐 코드·버그 먼저냐)·순서·상세 절차는 `product/ai-ops.md`의 '1. 운영 루프'를 **정본으로** 따릅니다 — 순서를 여기 따로 적지 않습니다(두 곳에 적으면 어긋나므로).
