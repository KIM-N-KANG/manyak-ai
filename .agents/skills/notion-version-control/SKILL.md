...
대부분의 파일은 독립된 데이터베이스를 가지지만, `chat-spec`만 예외로 명세 문서 4종이
단일 `Chat Spec Versions` DB를 공유한다(파일은 `Name` prefix로 구분). 새 버전 저장 시
**같은 파일의** 기존 `active` 항목을 `archived`로 전환하고 새 행을 `active`로 추가한다.
...
│
├── chat-spec
│   └── Chat Spec Versions (DB)   ← 명세 문서 전체(1~4)를 한 DB에 모아 저장 (chat과 달리 레이어별 분리 없음)
│
└── story-spec-json-field   ← 일반 문서 페이지(DB 아님). 버전 스냅샷 대상 아님 — 쓰기 금지.
```

> **쓰기 허용 범위 (엄수).** 이 스킬이 버전 행을 생성·수정할 수 있는 대상은
> `story`, `chat`, `story-spec`, `chat-spec` 네 페이지의 하위 데이터베이스뿐이다.
> `story-spec-json-field`를 비롯한 그 외 페이지·문서에는 어떤 내용도 쓰지 않는다(읽기 전용).
> 루트(`prompt/spec-version-control`)에 새 페이지를 만들거나 기존 문서 페이지를 수정하지 않는다.
...
#### 5-1. 같은 파일의 기존 active 항목 archived로 전환

해당 데이터베이스에서 **같은 파일(동일 `Name` prefix)의** `Status = active` 항목을 찾아
`notion-update-page`로 `Status`를 `archived`로 변경한다. `chat-spec`처럼 한 DB에 여러
파일이 공존하는 경우, **다른 파일의 active 항목은 건드리지 않는다.**
