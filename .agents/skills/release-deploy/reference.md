...
**키를 손으로 만지지 않는다.** `qa.sh`가 `.env`에 키가 없으면 Secrets Manager에서 받아 넣고, 끝나면 원상복구한다. 값은 변수에만 담기고 화면·로그에 찍히지 않는다.
...
**`qa.sh`는 키가 없으면 Secrets Manager에서 가져와 `.env`에 넣고, 끝나면 원상복구한다.** 값은 어디에도 출력하지 않는다(`test.sh`가 docker 인자를 화면에 그대로 찍기 때문에 `-e`로 넘기면 키가 노출된다 — 그래서 `.env` 경유다).
