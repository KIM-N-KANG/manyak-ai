...
source "$STATE"
: "${REPO:?상태 파일에 REPO가 없습니다}" "${PREV_REVIEWS:?}" "${PREV_COMMENTS:?}" "${CMTID:?}"
...
for i in $(seq 1 "$ROUNDS"); do
  ...
  rv=$(codex_count "repos/$REPO/pulls/$PR/reviews?per_page=100"   "[.[]|$BOT_FILTER]|length")   || rv=$PREV_REVIEWS
  cm=$(codex_count "repos/$REPO/issues/$PR/comments?per_page=100" "[.[]|$BOT_FILTER]|length")   || cm=$PREV_COMMENTS
  # thumbs-up은 반드시 봇 것만 — 사람이 먼저 누르면 오탐한다
  tu=$(codex_count "repos/$REPO/issues/comments/$CMTID/reactions?per_page=100" \
       "[.[]|select(.content==\"+1\")|$BOT_FILTER]|length") || tu=0
