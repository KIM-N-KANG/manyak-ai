...
prod일 때만 켠다(design/3-ai-server-design.md §3-3). 미충족이면 기동을 막지 않고 no-op + 오류 로그 — 관측
...
def init_langfuse() -> None:
    ...
    if host != _ALLOWED_HOST:
        logger.error(
            ...
            "(원문 수집은 JP 리전만 허용, design/3-ai-server-design.md §3-3)",
            _ALLOWED_HOST,
    ...
    if settings.sentry_environment != _ALLOWED_ENVIRONMENT:
        logger.error(
            ...
            "(원문 수집은 prod 전용, design/3-ai-server-design.md §3-3. 로컬 검증은 .env 환경값을 의식적으로 변경)",
...
def dimension_tags(
    *,
    ...
    후속, spec/5-ai-server-spec.md §5-6). story `genre_tags`는 키워드 단계 개편(KNK-621)이 커스텀 장르
    입력을 400으로 차단하면 사전 정의만 남는다 — 켜기는 그 배포 이후로 순서를 잡는다.
    주인공·주변 인물 특징(인물 세트 `features`, KNK-833)은 사용자가 직접 키워드를 입력해
    추가할 수 있어(US-3-3, spec/4-backend-server-spec.md §4-4) **원문이 섞이고 카디널리티가 폭발**한다 — 그래서
